...
6. **seed を準備する。** `node apps/demo/scripts/seed-manifest.mjs` は local manifest を検証します。remote を選ぶなら必ず `node apps/demo/scripts/seed-manifest.mjs --remote --target production` と明記します。これは現時点では manifest/target 検証のみで、resource に書き込みません。実 importer command は integration で接続予定です。接続後も production target を明示し、checksum report、interruption resume、attribution preservation を確認してから進みます。
...
- **不適切/削除要求:** 公開 admin endpoint を作らず、運用者 purge を実行します。reactive-only であり、自動検出・審査・通報ではありません。
